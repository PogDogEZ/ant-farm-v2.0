...
class BaseConsole(QFrame):
    ...
    def __init__(self, parent=None, formats=None):
        ...
        edit.setFrameStyle(QFrame.Shape.NoFrame)

        font = QFont("Source Code Pro SemiBold", 10)
        font.setFamily("Source Code Pro SemiBold")
        font_width = QFontMetrics(font).boundingRect('M').width()
        self.setFont(font)
        ...
        pbar.setFocusPolicy(Qt.FocusPolicy.NoFocus)
    ...
    def _get_key_event_handlers(self):
        return {
            ...
            Qt.Key.Key_Return:      self._handle_enter_key,
            Qt.Key.Key_Enter:       self._handle_enter_key,
            Qt.Key.Key_Backspace:   self._handle_backspace_key,
            Qt.Key.Key_Delete:      self._handle_delete_key,
            Qt.Key.Key_Home:        self._handle_home_key,
            Qt.Key.Key_Tab:         self._handle_tab_key,
            Qt.Key.Key_Backtab:     self._handle_backtab_key,
            Qt.Key.Key_Up:          self._handle_up_key,
            Qt.Key.Key_Down:        self._handle_down_key,
            Qt.Key.Key_Left:        self._handle_left_key,
            Qt.Key.Key_D:           self._handle_d_key,
            Qt.Key.Key_C:           self._handle_c_key,
            Qt.Key.Key_V:           self._handle_v_key,
            Qt.Key.Key_U:           self._handle_u_key,
        }
    ...
    def _filter_mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.MiddleButton:
            ...
            mime_data = clipboard.mimeData(QClipboard.Mode.Selection)
        ...
        if not cursor.hasSelection() and offset >= 1:
            ...
            if event.modifiers() == Qt.KeyboardModifier.ControlModifier:
                ...
                cursor.movePosition(
                    ...
                    QTextCursor.MoveMode.KeepAnchor, num)
    ...
    def _indent_selection(self, cursor, indent=True):
        ...
        cursor.setPosition(self._prompt_pos + pos1, QTextCursor.MoveMode.KeepAnchor)
        return cursor
    ...
    def _handle_up_key(self, event):
        ...
        if shift or '\n' in self.input_buffer()[:self.cursor_offset()]:
            self._move_cursor(QTextCursor.MoveOperation.Up, select=shift)
        else:
    ...
    def _handle_down_key(self, event):
        shift = event.modifiers() & Qt.KeyboardModifier.ShiftModifier
        if shift or '\n' in self.input_buffer()[self.cursor_offset():]:
            self._move_cursor(QTextCursor.MoveOperation.Down, select=shift)
        else:
    ...
    def _handle_d_key(self, event):
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier and not self.input_buffer():
    ...
    def _handle_u_key(self, event):
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier and self.input_buffer():
            self.clear_input_buffer()
            return True
        return False
